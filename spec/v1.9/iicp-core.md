...
| Field | Type | Constraint |
|-------|------|-----------|
| `task_id` | UUID v4 | Generated by proxy; identifies one logical task. An unprofiled base CALL has one attempt. Lifecycle retries retain `task_id` and use a new attempt `call_id` [→ DIR-HB-02]. |
...
| `constraints.timeout_ms` | integer | MUST be > 0; provider attempt budget measured from provider receipt through admission, execution and terminal response creation. It is not a delivery lifetime, absolute task deadline, result-retention promise or caller wait timeout [→ TASK-4]. |
