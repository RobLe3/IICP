...
function cosineWithNorms(a, aNorm, b, bNorm) {
...
const exampleNormsCache = new WeakMap();
function exampleNorms(candidate) {
    ...
    if (!norms || norms.length !== candidate.examples.length) {
        ...
        for (let i = 0; i < candidate.examples.length; i++)
            norms[i] = l2norm(candidate.examples[i].embedding);
...
export class Router {
    ...
    constructor(opts) {
        if (!opts.candidates?.length)
            throw new Error('Router needs at least one candidate');
     ...
     */
    static fromExamples(rows, prices, opts = {}) {
        const ids = new Set();
        for (const r of rows)
    ...
    predict(candidate, queryEmbedding) {
     ...
     *  for the SAME query. */
    predictWithQueryNorm(candidate, queryEmbedding, queryNorm) {
        const examples = candidate.examples;
        if (examples.length === 0)
            return 0;
        const norms = exampleNorms(candidate);
        const scored = new Array(examples.length);
        for (let i = 0; i < examples.length; i++) {
            ...
            const score = e.length === queryEmbedding.length
