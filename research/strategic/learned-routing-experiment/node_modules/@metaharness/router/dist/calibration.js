 ...
 */
export function reliabilityBins(pairs, binCount = DEFAULT_BIN_COUNT) {
    ...
    const sumPredicted = new Array(binCount).fill(0);
    const sumRealized = new Array(binCount).fill(0);
    for (const { predicted, realized } of pairs) {
...
export function expectedCalibrationError(pairs, binCount = DEFAULT_BIN_COUNT) {
    ...
    let weighted = 0;
    for (const b of bins)
        weighted += b.count * Math.abs(b.gap);
 ...
 */
export function calibrationReport(pairs, binCount = DEFAULT_BIN_COUNT) {
    ...
    for (const b of bins) {
        if (b.count === 0)
            continue;
        if (worstBin === null || Math.abs(b.gap) > Math.abs(worstBin.gap))
            worstBin = b;
    }
    return {
